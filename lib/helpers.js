...
// ── OUTBOUND BLOCKLIST ───────────────────────────────────────────────────────
// True if the email address or its whole domain is in blocked_senders.
// This is the permanent firewall: it applies even to contacts/companies
// created later, since it keys on the address itself.
async function emailBlocked(email) {
  const e = (email || '').toLowerCase().trim()
  if (!e.includes('@')) return false
  const domain = '@' + e.split('@')[1]
  const row = await one('SELECT 1 AS hit FROM blocked_senders WHERE pattern = $1 OR pattern = $2 LIMIT 1', [e, domain])
  return !!row
}

module.exports = {
  run, one, all, emailBlocked,
